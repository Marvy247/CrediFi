...
export function CreditScoreDashboard() {
  ...
  const { data: creditHistory, refetch: refetchHistory } = useReadContract({
    address: contracts.creditScoreOracle,
    abi: CreditScoreOracleABI,
  ...
  const score = creditScore?.score !== undefined ? Number(creditScore.score) : (creditScore?.[0] ? Number(creditScore[0]) : 0)
  const ltvRatio = creditScore?.ltvRatio !== undefined ? Number(creditScore.ltvRatio) : (creditScore?.[1] ? Number(creditScore[1]) : 50)
  const interestRate = creditScore?.interestRate !== undefined ? Number(creditScore.interestRate) / 100 : (creditScore?.[2] ? Number(creditScore[2]) / 100 : 10)

  const totalLoans = creditHistory?.totalLoans !== undefined ? Number(creditHistory.totalLoans) : (creditHistory?.[0] ? Number(creditHistory[0]) : 0)
  const successfulRepayments = creditHistory?.successfulRepayments !== undefined ? Number(creditHistory.successfulRepayments) : (creditHistory?.[1] ? Number(creditHistory[1]) : 0)
  const totalBorrowed = creditHistory?.totalBorrowed !== undefined ? Number(creditHistory.totalBorrowed) : (creditHistory?.[2] ? Number(creditHistory[2]) : 0)
  const totalRepaid = creditHistory?.totalRepaid !== undefined ? Number(creditHistory.totalRepaid) : (creditHistory?.[3] ? Number(creditHistory[3]) : 0)
    ...
    <div className="space-y-6">
      <div className="bg-white rounded-2xl p-8 border border-gray-200 shadow-sm">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-bold text-gray-900">Credit Score Overview</h2>
        ...
        <div className="grid md:grid-cols-3 gap-6">
          ...
          <div className="text-center p-6 bg-green-50 rounded-xl border border-green-200">
            <div className="text-5xl font-bold text-green-600">{ltvRatio}%</div>
          ...
          <div className="text-center p-6 bg-orange-50 rounded-xl border border-orange-200">
            <div className="text-5xl font-bold text-orange-600">{interestRate}%</div>
        ...
        {creditHistory && (
          <div className="grid md:grid-cols-2 gap-4">
            <div className="flex justify-between items-center p-4 bg-gray-50 rounded-lg">
              ...
              <span className="text-2xl font-bold text-gray-900">{totalLoans}</span>
            </div>
            <div className="flex justify-between items-center p-4 bg-green-50 rounded-lg border border-green-200">
              <span className="text-gray-600 font-medium">Successful Repayments</span>
              <span className="text-2xl font-bold text-green-600">{successfulRepayments}</span>
            </div>
            <div className="flex justify-between items-center p-4 bg-gray-50 rounded-lg">
              <span className="text-gray-600 font-medium">Total Borrowed</span>
              <span className="text-2xl font-bold text-gray-900">{(totalBorrowed / 1e18).toFixed(4)} ETH</span>
            </div>
            <div className="flex justify-between items-center p-4 bg-gray-50 rounded-lg">
              <span className="text-gray-600 font-medium">Total Repaid</span>
              <span className="text-2xl font-bold text-gray-900">{(totalRepaid / 1e18).toFixed(4)} ETH</span>
            </div>
          </div>
        )}

        {!creditHistory || totalLoans === 0 && (
